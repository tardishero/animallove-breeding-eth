  ...
  const deposit = async () => {
    ...
    if (isConnected) {
      if (aaselectedtokenID === "" || ppselectedtokenID === "") {
        ...
        const sgbValue = ethers.BigNumber.from(sgbamount).mul(ethers.BigNumber.from(10).pow(21))
  ...
  return (
    <ScrollAnimation animateIn="fadeIn">
      ...
      <Row gutter={[24, 24]} justify={"center"} className="headerBar">
        ...
        <h1 style={{ color: "white", textShadow: " 0px 2px 20px black", fontWeight: "400", fontSize: '20px', textAlign: 'center' }}>Trip#4 has been completed.</h1>
        <h1 style={{ color: "white", textShadow: " 0px 2px 20px black", fontWeight: "400", fontSize: '20px', textAlign: 'center' }}>Aliens should be home by now.</h1>
        <h1 style={{ color: "white", textShadow:" 0px 2px 20px black", fontWeight: "400", fontSize: '20px', textAlign: 'center' }}>Travel program will be on paused from now.</h1>
        <Row gutter={[24, 24]} justify={"center"} className="headerBar">
          <Col lg={9} xs={2}></Col>
          ...
          <Col lg={6} xs={15} className="headerBarItem-DAPP">
            {!isConnected ? (
              <>
                <button className="btn-connect" Click={connect}>
                  Connect
                </button>
              </>
            ) : (
